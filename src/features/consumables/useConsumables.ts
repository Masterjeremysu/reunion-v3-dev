...
export function useCreateConsumable() {
  ...
  return useMutation({
    mutationFn: async (payload: Omit<ConsumableRequestInsert, 'organization_id'>) => {
      ...
      const { data, error } = await (supabase.from('consumable_requests') as any).insert({ ...payload, organization_id: organization.id }).select().single()
      if (error) throw error
      return data as any[]
    },
...
export function useUpdateConsumableStatus() {
  ...
  return useMutation({
    mutationFn: async ({ id, status }: { id: string; status: string }) => {
      const { data, error } = await (supabase.from('consumable_requests') as any).update({ status }).eq('id', id).select().single()
      if (error) throw error
      return data as any[]
    },
...
export function useAddMood() {
  ...
  return useMutation({
    mutationFn: async (payload: Omit<TeamMoodInsert, 'organization_id'>) => {
      if (!organization?.id) throw new Error("Organisation introuvable")
      const { data, error } = await (supabase.from('team_mood') as any).insert({ ...payload, organization_id: organization.id }).select().single()
      if (error) throw error
      return data
    },
