...
export function useCreateAction() {
  ...
  const { organization } = useAuth()
  return useMutation({
    mutationFn: async (payload: Omit<ActionItemInsert, 'organization_id'>) => {
      if (!organization?.id) throw new Error("Organisation introuvable")
      const { data, error } = await (supabase.from('action_items') as any).insert({ ...payload, organization_id: organization.id }).select().single()
      if (error) throw error
      return data
    },
    onSuccess: (data) => {
      qc.invalidateQueries({ queryKey: QK.ACTIONS })
      if ((data as any).meeting_id) qc.invalidateQueries({ queryKey: QK.MEETING_ACTIONS((data as any).meeting_id) })
...
export function useUpdateAction() {
  ...
  return useMutation({
    mutationFn: async ({ id, ...payload }: ActionItemUpdate & { id: string }) => {
      const { data, error } = await (supabase.from('action_items') as any).update(payload).eq('id', id).select().single()
      if (error) throw error
      return data
    },
    onSuccess: (data) => {
