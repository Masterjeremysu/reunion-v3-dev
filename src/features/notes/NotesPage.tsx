...
function NoteEditor({ note, meetings, onSave, onClose }: {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      {/* Editor topbar */}
      <div style={{ padding: '14px 20px', borderBottom: '1px solid var(--color-border)', display: 'flex', alignItems: 'center', gap: 10, flexShrink: 0 }}>
        <div style={{ flex: 1 }}>
          <input value={title} onChange={e => setTitle(e.target.value)} placeholder="Titre de la note..."
            style={{ width: '100%', background: 'transparent', border: 'none', fontSize: 16, fontWeight: 700, color: 'var(--color-text-main)', outline: 'none', fontFamily: 'inherit' }} />
        </div>
        <div style={{ display: 'flex', gap: 6 }}>
          <button type="button" onClick={insertCheckbox} title="Insérer une case à cocher"
            style={{ padding: 7, background: 'var(--color-bg-input)', border: '1px solid var(--color-border)', borderRadius: 7, cursor: 'pointer', color: 'var(--color-text-muted)', display: 'flex' }}>
          ...
          <button type="button" onClick={handleSave} disabled={saving || !title.trim()}
            style={{ padding: '7px 14px', background: '#1D9E75', border: 'none', borderRadius: 7, cursor: 'pointer', color: 'var(--color-text-main)', fontSize: 12, fontWeight: 600, display: 'flex', alignItems: 'center', gap: 5, opacity: title.trim() ? 1 : 0.4 }}>
      ...
      {/* Meta row */}
      <div style={{ padding: '10px 20px', borderBottom: '1px solid var(--color-border)', display: 'flex', alignItems: 'center', gap: 12, flexShrink: 0, flexWrap: 'wrap' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          <CalendarDays style={{ width: 12, height: 12, color: 'var(--color-text-muted)' }} />
          <span style={{ fontSize: 11, color: 'var(--color-text-muted)', fontFamily: 'monospace' }}>Lier à la réunion du</span>
          <input type="date" value={forDate} onChange={e => setForDate(e.target.value)}
            style={{ background: 'var(--color-bg-input)', border: '1px solid var(--color-border)', borderRadius: 6, padding: '2px 8px', fontSize: 11, color: 'var(--color-text-main)', outline: 'none', fontFamily: 'monospace' }} />
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 5, flexWrap: 'wrap' }}>
          {tags.map(t => {
            ...
            return <TagPill key={t} label={t} color={preset?.color ?? 'var(--color-text-muted)'} onRemove={() => removeTag(t)} />
          })}
          <div style={{ position: 'relative' }}>
            <input value={tagInput} onChange={e => setTagInput(e.target.value)}
              onKeyDown={e => { if (e.key === 'Enter' && tagInput.trim()) addTag(tagInput.trim()) }}
              placeholder="+ Tag"
              style={{ width: 60, background: 'var(--color-bg-input)', border: '1px solid var(--color-border)', borderRadius: 20, padding: '2px 8px', fontSize: 10, color: 'var(--color-text-muted)', outline: 'none', fontFamily: 'monospace' }} />
          </div>
      ...
      <div style={{ flex: 1, overflow: 'hidden', position: 'relative' }}>
        <textarea
          ...
          style={{ width: '100%', height: '100%', background: 'transparent', border: 'none', padding: '20px', fontSize: 14, color: 'var(--color-text-main)', outline: 'none', resize: 'none', lineHeight: 1.7, fontFamily: "'DM Mono', 'JetBrains Mono', monospace", boxSizing: 'border-box' }}
        />
      </div>

      {/* Footer */}
      <div style={{ padding: '8px 20px', borderTop: '1px solid var(--color-border)', display: 'flex', alignItems: 'center', gap: 12, flexShrink: 0 }}>
        <span style={{ fontSize: 10, color: 'var(--color-text-faded)', fontFamily: 'monospace' }}>{wc} mot{wc > 1 ? 's' : ''}</span>
        <span style={{ fontSize: 10, color: 'var(--color-text-faded)', fontFamily: 'monospace' }}>{content.length} car.</span>
        <span style={{ fontSize: 10, color: 'var(--color-text-faded)', fontFamily: 'monospace', marginLeft: 'auto' }}>Ctrl+Enter pour sauvegarder</span>
      </div>
    </div>
  )
}
...
function NoteCard({ note, meetings, isSelected, onClick }: { note: any; meetings: any[]; isSelected: boolean; onClick: () => void }) {
  ...
  return (
    ...
    onMouseEnter={e => { if (!isSelected) (e.currentTarget as HTMLElement).style.background = 'var(--color-bg-input)' }}
    onMouseLeave={e => { if (!isSelected) (e.currentTarget as HTMLElement).style.background = 'transparent' }}>
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10 }}>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 3 }}>
            <span style={{ fontSize: 13, fontWeight: 600, color: note.is_archived ? 'var(--color-text-muted)' : 'var(--color-text-main)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', fontStyle: note.is_archived ? 'italic' : 'normal' }}>
              {note.title}
            </span>
          ...
          {preview && <p style={{ fontSize: 11, color: 'var(--color-text-muted)', margin: '0 0 5px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', lineHeight: 1.4, fontFamily: 'monospace' }}>{preview}...</p>}
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', alignItems: 'center' }}>
            <span style={{ fontSize: 10, color: 'var(--color-text-faded)', fontFamily: 'monospace' }}>{fRelative(note.created_at)}</span>
            ...
            {(note.tags ?? []).slice(0, 2).map((t: string) => {
              ...
              return <TagPill key={t} label={t} color={p?.color ?? 'var(--color-text-muted)'} small />
            })}
          </div>
        </div>
      </div>
      ...
      if (line.startsWith('☐ ') || line.startsWith('☑ ')) {
        ...
        return (
          <div key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: 8, marginBottom: 4 }}>
            <span style={{ fontSize: 14, color: checked ? '#1D9E75' : 'var(--color-text-muted)', flexShrink: 0, marginTop: 1 }}>{checked ? '☑' : '☐'}</span>
            <span style={{ fontSize: 14, color: checked ? 'var(--color-text-muted)' : 'var(--color-text-main)', textDecoration: checked ? 'line-through' : 'none', lineHeight: 1.6 }}>{line.slice(2)}</span>
          </div>
        )
      }
      if (line.startsWith('# ')) return <p key={i} style={{ fontSize: 16, fontWeight: 700, color: 'var(--color-text-main)', margin: '12px 0 4px' }}>{line.slice(2)}</p>
      if (line.startsWith('## ')) return <p key={i} style={{ fontSize: 14, fontWeight: 600, color: 'var(--color-text-main)', margin: '10px 0 3px' }}>{line.slice(3)}</p>
      if (line === '') return <div key={i} style={{ height: 8 }} />
      return <p key={i} style={{ fontSize: 14, color: 'var(--color-text-main)', margin: '0 0 4px', lineHeight: 1.65, fontFamily: "'DM Mono', monospace" }}>{line}</p>
    })
  }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: 'var(--color-bg-app)', overflow: 'hidden' }}>
      ...
      {deleteConfirm && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 50, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(0,0,0,0.75)' }}>
          <div style={{ background: 'var(--color-bg-card)', border: '1px solid var(--color-border2)', borderRadius: 16, padding: 24, maxWidth: 340, width: '100%', margin: '0 16px' }}>
            <h3 style={{ fontSize: 14, fontWeight: 600, color: 'var(--color-text-main)', marginBottom: 8 }}>Supprimer cette note ?</h3>
            <p style={{ fontSize: 12, color: 'var(--color-text-muted)', marginBottom: 20 }}>Irréversible.</p>
            <div style={{ display: 'flex', gap: 10 }}>
              ...
              <button onClick={() => handleDelete(deleteConfirm)} style={{ flex: 1, padding: '9px 0', fontSize: 13, fontWeight: 600, color: 'var(--color-text-main)', background: '#E24B4A', border: 'none', borderRadius: 10, cursor: 'pointer' }}>Supprimer</button>
            </div>
          </div>
        </div>
      )}
      ...
      <div style={{ flexShrink: 0, padding: '0 24px', height: 52, borderBottom: '1px solid var(--color-border)', background: 'var(--color-bg-sidebar)', display: 'flex', alignItems: 'center', gap: 12 }}>
        <h1 style={{ fontSize: 14, fontWeight: 600, color: 'var(--color-text-main)', margin: 0 }}>Notes de préparation</h1>
        <div style={{ display: 'flex', gap: 8 }}>
          {[
            { label: `${stats.total} notes`, color: '#1D9E75' },
            stats.withMeeting > 0 && { label: `${stats.withMeeting} liées`, color: '#378ADD' },
            stats.archived > 0 && { label: `${stats.archived} archivées`, color: 'var(--color-text-faded)' },
        ...
        <div style={{ marginLeft: 'auto', display: 'flex', gap: 8 }}>
          <button onClick={() => setShowArchived(!showArchived)}
            style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '6px 12px', background: showArchived ? 'var(--color-border)' : 'var(--color-border)', border: '1px solid var(--color-border2)', borderRadius: 8, color: 'var(--color-text-muted)', fontSize: 12, cursor: 'pointer' }}>
      ...
      <div style={{ display: 'flex', flex: 1, minHeight: 0 }}>
        {/* ── Left list ── */}
        <div style={{ width: 280, flexShrink: 0, borderRight: '1px solid var(--color-border)', display: 'flex', flexDirection: 'column' }}>
          {/* Search + tags */}
            ...
            {allTags.length > 0 && (
              <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
                <button onClick={() => setFilterTag(null)}
                  style={{ padding: '2px 8px', background: !filterTag ? 'var(--color-border)' : 'transparent', border: '1px solid var(--color-border2)', borderRadius: 20, fontSize: 10, color: filterTag ? 'var(--color-text-faded)' : 'var(--color-text-main)', cursor: 'pointer', fontFamily: 'monospace' }}>
                  Toutes
                </button>
                {allTags.map(t => {
                  const p = TAG_PRESETS.find(x => x.label === t)
                  return (
                    <button key={t} onClick={() => setFilterTag(filterTag === t ? null : t)}
                      style={{ padding: '2px 8px', background: filterTag === t ? `${p?.color ?? 'var(--color-text-muted)'}20` : 'transparent', border: `1px solid ${filterTag === t ? (p?.color ?? 'var(--color-text-muted)') : 'var(--color-border)'}30`, borderRadius: 20, fontSize: 10, color: p?.color ?? 'var(--color-text-muted)', cursor: 'pointer', fontFamily: 'monospace' }}>
                      {t}
                    </button>
                  )
                })}
        ...
        {/* ── Right panel ── */}
        <div style={{ flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column', background: 'var(--color-bg-app)' }}>
          {creating ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
              {/* Note header */}
              <div style={{ padding: '20px 28px 16px', borderBottom: '1px solid var(--color-border)', flexShrink: 0, background: 'linear-gradient(180deg, rgba(29,158,117,0.04) 0%, transparent 100%)' }}>
                <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12, marginBottom: 10 }}>
                  <div style={{ flex: 1 }}>
                    <h2 style={{ fontSize: 20, fontWeight: 700, color: 'var(--color-text-main)', margin: 0, letterSpacing: '-0.02em' }}>{selected.title}</h2>
                    <div style={{ display: 'flex', gap: 10, marginTop: 6, flexWrap: 'wrap', alignItems: 'center' }}>
                      <span style={{ fontSize: 11, color: 'var(--color-text-muted)', fontFamily: 'monospace', display: 'flex', alignItems: 'center', gap: 4 }}>
                      ...
                      {(selected.tags ?? []).map((t: string) => {
                        const p = TAG_PRESETS.find(x => x.label === t)
                        return <TagPill key={t} label={t} color={p?.color ?? 'var(--color-text-muted)'} />
                      })}
                    </div>
                  </div>
                  <div style={{ display: 'flex', gap: 6 }}>
                    <button onClick={() => setEditing(true)} title="Modifier"
                      style={{ padding: 8, background: 'var(--color-bg-input)', border: '1px solid var(--color-border)', borderRadius: 8, cursor: 'pointer', color: 'var(--color-text-muted)', display: 'flex', transition: 'all 0.15s' }}
                      onMouseEnter={e => { (e.currentTarget as HTMLElement).style.color = 'var(--color-text-main)'; (e.currentTarget as HTMLElement).style.borderColor = 'var(--color-border2)' }}
              ...
              <div style={{ flex: 1, overflowY: 'auto', padding: '24px 28px' }}>
                {selected.content ? (
                  ...
                  <div style={{ textAlign: 'center', padding: '40px 0', color: 'var(--color-text-faded)' }}>
            ...
            <div style={{ height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 14, color: 'var(--color-text-faded)' }}>
              <FileText style={{ width: 40, height: 40, opacity: 0.2 }} />
              <div style={{ textAlign: 'center' }}>
                <p style={{ fontSize: 14, margin: '0 0 6px' }}>Sélectionnez une note</p>
                <p style={{ fontSize: 12, color: 'var(--color-text-faded)' }}>ou créez-en une nouvelle</p>
              </div>
              <button onClick={() => setCreating(true)}
                style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '8px 16px', background: '#1D9E75', border: 'none', borderRadius: 10, color: 'var(--color-text-main)', fontSize: 13, cursor: 'pointer' }}>
