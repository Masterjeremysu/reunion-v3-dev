...
import {
  ...
  Loader2, ChevronRight, Activity, Award,
...
function ColleagueAvatar({ name, size = 40, inactive }: { name: string; size?: number; inactive?: boolean }) {
  ...
  return (
    <div style={{ width: size, height: size, borderRadius: '50%', background: inactive ? 'rgba(255,255,255,0.05)' : bg, border: `1.5px solid ${inactive ? 'rgba(255,255,255,0.1)' : color + '40'}`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: size * 0.28, fontWeight: 700, color: inactive ? 'rgba(255,255,255,0.25)' : color, flexShrink: 0, letterSpacing: '-0.02em', filter: inactive ? 'grayscale(1)' : 'none', position: 'relative' }}>
      {initials}
      {inactive && (
        <div style={{ position: 'absolute', bottom: -1, right: -1, width: size * 0.32, height: size * 0.32, borderRadius: '50%', background: '#565c75', border: '1.5px solid #0a0c12', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <UserX style={{ width: size * 0.16, height: size * 0.16, color: '#fff' }} />
        </div>
      )}
    </div>
  )
  ...
  if (editing) {
    if (multiline) return (
      <textarea value={val} onChange={e => setVal(e.target.value)} autoFocus onBlur={save}
        ...
        rows={3} style={{ width: '100%', background: '#1e2535', border: '1px solid #1D9E75', borderRadius: 6, padding: '4px 8px', fontSize: 'inherit', color: '#fff', outline: 'none', resize: 'none', fontFamily: 'inherit', ...s }} />
    )
    return (
...
function ActivityTimeline({ meetings, actions }: { meetings: any[]; actions: any[] }) {
  ...
  if (!events.length) return <p style={{ fontSize: 12, color: 'rgba(255,255,255,0.2)', textAlign: 'center', padding: '16px 0' }}>Aucune activité récente</p>
  return (
    <div style={{ position: 'relative', paddingLeft: 20 }}>
      ...
      {events.map((e, i) => (
        <div key={e.id + i} style={{ display: 'flex', alignItems: 'flex-start', gap: 12, marginBottom: 12, position: 'relative' }}>
          <div style={{ position: 'absolute', left: -13, top: 4, width: 8, height: 8, borderRadius: '50%', background: e.type === 'meeting' ? '#1D9E75' : '#378ADD', border: '2px solid #0a0c12' }} />
...
function InactivateModal({ colleague, onClose }: { colleague: any; onClose: () => void }) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 50, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(0,0,0,0.75)' }}>
      <div style={{ background: '#161b26', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 16, padding: 28, maxWidth: 400, width: '100%', margin: '0 16px' }}>
        <div style={{ display: 'flex', gap: 12, marginBottom: 16 }}>
          ...
          <div>
            <h3 style={{ fontSize: 15, fontWeight: 700, color: '#fff', margin: 0 }}>Inactiver {colleague.name}</h3>
            <p style={{ fontSize: 12, color: 'rgba(255,255,255,0.4)', margin: '3px 0 0' }}>Les données historiques sont conservées</p>
          </div>
        </div>

        <div style={{ display: 'flex', flexDirection: 'column', gap: 12, marginBottom: 20 }}>
          <div>
            <label style={{ display: 'block', fontSize: 10, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.35)', fontFamily: 'monospace', marginBottom: 5 }}>Date de départ</label>
            <input type="date" value={date} onChange={e => setDate(e.target.value)}
              style={{ width: '100%', background: '#1e2535', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 8, padding: '9px 12px', fontSize: 13, color: '#fff', outline: 'none', boxSizing: 'border-box' }} />
          </div>
          <div>
            <label style={{ display: 'block', fontSize: 10, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.35)', fontFamily: 'monospace', marginBottom: 5 }}>Motif (optionnel)</label>
            <select value={reason} onChange={e => setReason(e.target.value)}
              style={{ width: '100%', background: '#1e2535', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 8, padding: '9px 12px', fontSize: 13, color: reason ? '#fff' : '#565c75', outline: 'none' }}>
        ...
        <div style={{ padding: '12px 14px', background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 10, marginBottom: 20 }}>
        ...
        <div style={{ display: 'flex', gap: 10 }}>
          ...
          <button onClick={handle} disabled={update.isPending}
            style={{ flex: 1, padding: '10px 0', fontSize: 13, fontWeight: 700, color: '#fff', background: '#EF9F27', border: 'none', borderRadius: 10, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
        <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 10, padding: '12px 14px' }}>
          <p style={{ fontSize: 9, color: 'rgba(255,255,255,0.25)', fontFamily: 'monospace', textTransform: 'uppercase', letterSpacing: '0.08em', margin: '0 0 5px' }}>Contrat</p>
          <select value={colleague.contract_type ?? 'CDI'}
            onChange={e => onUpdate({ contract_type: e.target.value })}
            style={{ background: 'transparent', border: 'none', fontSize: 13, fontWeight: 600, color: '#e8eaf0', outline: 'none', cursor: 'pointer', width: '100%' }}>
            {CONTRACT_TYPES.map(c => <option key={c} value={c} style={{ background: '#161b26' }}>{c}</option>)}
          </select>
        </div>
        <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 10, padding: '12px 14px' }}>
          <p style={{ fontSize: 9, color: 'rgba(255,255,255,0.25)', fontFamily: 'monospace', textTransform: 'uppercase', letterSpacing: '0.08em', margin: '0 0 5px' }}>Date d'entrée</p>
          <input type="date" value={colleague.hire_date ?? ''}
            onChange={e => onUpdate({ hire_date: e.target.value || null })}
            style={{ background: 'transparent', border: 'none', fontSize: 13, color: colleague.hire_date ? '#e8eaf0' : 'rgba(255,255,255,0.25)', outline: 'none', width: '100%', cursor: 'pointer' }} />
        </div>
      </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
        {[
          ...
          <div key={f.key} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '9px 12px', background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.05)', borderRadius: 8 }}>
            <f.icon style={{ width: 13, height: 13, color: 'rgba(255,255,255,0.3)', flexShrink: 0 }} />
            <EditableField
              value={colleague[f.key] ?? ''}
              placeholder={f.placeholder}
              onSave={v => onUpdate({ [f.key]: v || null })}
              style={{ fontSize: 13, color: colleague[f.key] ? 'rgba(255,255,255,0.75)' : 'rgba(255,255,255,0.25)', flex: 1 }}
            />
          </div>
        ))}
      </div>
      ...
      {/* Notes */}
      <div>
        <p style={{ fontSize: 9, color: 'rgba(255,255,255,0.25)', fontFamily: 'monospace', textTransform: 'uppercase', letterSpacing: '0.08em', margin: '0 0 6px' }}>Notes internes</p>
        <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.05)', borderRadius: 8, padding: '10px 12px', minHeight: 60 }}>
          <EditableField
            value={colleague.notes ?? ''}
            placeholder="Informations complémentaires, observations..."
            onSave={v => onUpdate({ notes: v || null })}
            multiline
            style={{ fontSize: 13, color: colleague.notes ? 'rgba(255,255,255,0.7)' : 'rgba(255,255,255,0.25)', width: '100%', display: 'block' }}
          />
        </div>
      </div>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: '#0a0c12', overflow: 'hidden' }}>
      ...
      {deleteConfirm && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 50, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(0,0,0,0.75)' }}>
          <div style={{ background: '#161b26', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 16, padding: 24, maxWidth: 340, width: '100%', margin: '0 16px' }}>
            <h3 style={{ fontSize: 14, fontWeight: 600, color: '#fff', marginBottom: 8 }}>Supprimer définitivement ?</h3>
            <p style={{ fontSize: 12, color: 'rgba(255,255,255,0.4)', marginBottom: 6 }}>Cette action est irréversible. Toutes les données liées seront orphelines.</p>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              ...
              <button onClick={async () => { await deleteColleague.mutateAsync(deleteConfirm); setDeleteConfirm(null); if (selectedId === deleteConfirm) setSelectedId(null) }}
                style={{ flex: 1, padding: '9px 0', fontSize: 13, fontWeight: 600, color: '#fff', background: '#E24B4A', border: 'none', borderRadius: 10, cursor: 'pointer' }}>
      ...
      <div style={{ flexShrink: 0, padding: '0 24px', height: 52, borderBottom: '1px solid rgba(255,255,255,0.05)', background: '#0e1118', display: 'flex', alignItems: 'center', gap: 12 }}>
        ...
        <div style={{ display: 'flex', gap: 6 }}>
          ...
          {inactiveCount > 0 && <span style={{ fontSize: 10, color: '#565c75', background: 'rgba(255,255,255,0.05)', borderRadius: 20, padding: '2px 8px', fontFamily: 'monospace' }}>{inactiveCount} inactif{inactiveCount > 1 ? 's' : ''}</span>}
        </div>
        <div style={{ marginLeft: 'auto', display: 'flex', gap: 8 }}>
          <button onClick={() => setShowInactive(!showInactive)}
            style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '6px 12px', background: showInactive ? 'rgba(255,255,255,0.08)' : 'rgba(255,255,255,0.04)', border: `1px solid ${showInactive ? 'rgba(255,255,255,0.2)' : 'rgba(255,255,255,0.07)'}`, borderRadius: 8, color: showInactive ? '#e8eaf0' : 'rgba(255,255,255,0.4)', fontSize: 12, cursor: 'pointer' }}>
            <Archive style={{ width: 12, height: 12 }} />
            {showInactive ? 'Voir actifs' : `Anciens (${inactiveCount})`}
          </button>
      ...
      <div style={{ display: 'flex', flex: 1, minHeight: 0 }}>
        ...
        {/* ── Liste gauche ── */}
        <div style={{ width: 300, flexShrink: 0, borderRight: '1px solid rgba(255,255,255,0.05)', display: 'flex', flexDirection: 'column' }}>
            ...
            <form onSubmit={handleCreate} style={{ padding: 12, borderBottom: '1px solid rgba(255,255,255,0.06)', background: '#0e1118', display: 'flex', flexDirection: 'column', gap: 8 }}>
              ...
              <div style={{ display: 'flex', gap: 6 }}>
                ...
                <button type="submit" disabled={!newName.trim() || !newPost.trim() || createColleague.isPending}
                  style={{ flex: 1, padding: '7px 0', fontSize: 12, fontWeight: 600, color: '#fff', background: '#1D9E75', border: 'none', borderRadius: 8, cursor: 'pointer', opacity: newName.trim() && newPost.trim() ? 1 : 0.4 }}>
            ...
            {filtered.map(c => {
              ...
              return (
                <button key={c.id} onClick={() => { setSelectedId(c.id); setActiveTab('overview') }}
                  className="col-item"
                  style={{ width: '100%', textAlign: 'left', padding: '12px 16px', borderBottom: '1px solid rgba(255,255,255,0.03)', borderLeft: `3px solid ${isSelected ? cColor : 'transparent'}`, background: isSelected ? `${cColor}08` : 'transparent', cursor: 'pointer', transition: 'all 0.12s' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                    <ColleagueAvatar name={c.name} size={36} inactive={inactive} />
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                        <span style={{ fontSize: 13, fontWeight: 600, color: inactive ? 'rgba(255,255,255,0.35)' : isSelected ? '#e8eaf0' : 'rgba(255,255,255,0.8)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{c.name}</span>
                        {!inactive && cLate > 0 && <span style={{ width: 14, height: 14, borderRadius: '50%', background: '#E24B4A', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 8, fontWeight: 700, color: '#fff', flexShrink: 0 }}>{cLate}</span>}
                        {inactive && <span style={{ fontSize: 9, color: '#565c75', background: 'rgba(255,255,255,0.05)', borderRadius: 20, padding: '1px 5px', fontFamily: 'monospace', flexShrink: 0 }}>inactif</span>}
                      </div>
                      ...
                      {inactive && c.inactive_since && (
                        <p style={{ fontSize: 10, color: 'rgba(255,255,255,0.2)', margin: '3px 0 0', fontFamily: 'monospace' }}>
        ...
        <div style={{ flex: 1, overflowY: 'auto', background: '#0c0f18' }}>
          {!selected ? (
            ...
            <div>
              {/* Hero */}
              <div style={{ padding: '28px 32px 20px', borderBottom: '1px solid rgba(255,255,255,0.05)', background: `linear-gradient(135deg, ${isInactive ? 'rgba(255,255,255,0.02)' : accentColor + '08'} 0%, transparent 60%)`, position: 'relative' }}>
                {isInactive && (
                  <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: 2, background: 'linear-gradient(90deg, #565c75, transparent)', opacity: 0.5 }} />
                )}
                <div style={{ display: 'flex', alignItems: 'flex-start', gap: 18, marginBottom: 16 }}>
                  ...
                  <div style={{ flex: 1 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 4 }}>
                      <EditableField value={selected.name} onSave={v => handleUpdate({ name: v })}
                        style={{ fontSize: 20, fontWeight: 700, color: isInactive ? 'rgba(255,255,255,0.45)' : '#fff', letterSpacing: '-0.02em' }} />
                      {isInactive && <span style={{ fontSize: 11, color: '#8b90a4', background: 'rgba(255,255,255,0.06)', borderRadius: 20, padding: '2px 10px', fontFamily: 'monospace' }}>Inactif</span>}
                  ...
                  <div style={{ display: 'flex', gap: 6 }}>
                    ...
                    <button onClick={() => setDeleteConfirm(selected.id)}
                      style={{ padding: 8, background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 8, cursor: 'pointer', color: 'rgba(255,255,255,0.3)', display: 'flex' }}
                ...
                {/* Inactif banner */}
                {isInactive && (
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '12px 16px', background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 10 }}>
                    <Archive style={{ width: 14, height: 14, color: '#8b90a4', flexShrink: 0 }} />
              ...
              <div style={{ display: 'flex', borderBottom: '1px solid rgba(255,255,255,0.05)', padding: '0 32px' }}>
                {TABS.map(tab => (
                  <button key={tab.key} onClick={() => setActiveTab(tab.key as any)}
                    style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '12px 16px', fontSize: 12, border: 'none', cursor: 'pointer', background: 'transparent', borderBottom: `2px solid ${activeTab === tab.key ? accentColor : 'transparent'}`, color: activeTab === tab.key ? accentColor : 'rgba(255,255,255,0.35)', transition: 'all 0.15s', marginBottom: -1 }}>
              ...
              <div style={{ padding: '20px 32px' }}>
                ...
                {activeTab === 'overview' && stats && (
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
                    <div>
                      <p style={{ fontSize: 10, fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.25)', fontFamily: 'monospace', marginBottom: 12 }}>Activité récente</p>
                      <ActivityTimeline meetings={stats.myMeetings} actions={stats.myActions} />
                    </div>
                    <div>
                      <p style={{ fontSize: 10, fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.25)', fontFamily: 'monospace', marginBottom: 12 }}>Actions en cours</p>
                      ...
                      {stats.myActions.map(a => {
                        ...
                        return (
                          <div key={a.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 14px', background: 'rgba(255,255,255,0.02)', border: `1px solid ${late ? '#E24B4A20' : 'rgba(255,255,255,0.05)'}`, borderRadius: 10 }}>
                            ...
                            <span style={{ fontSize: 10, color: 'rgba(255,255,255,0.3)', background: 'rgba(255,255,255,0.05)', borderRadius: 20, padding: '2px 8px', fontFamily: 'monospace', flexShrink: 0 }}>
                      ...
                      {stats.myMeetings.sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()).map(m => {
                        ...
                        return (
                          <div key={m.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 14px', background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.05)', borderRadius: 10 }}>
                            <div style={{ width: 36, height: 36, borderRadius: 8, background: 'rgba(255,255,255,0.04)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                            ...
                            <div style={{ flex: 1, minWidth: 0 }}>
                              ...
                              <p style={{ fontSize: 10, color: 'rgba(255,255,255,0.3)', margin: '2px 0 0', fontFamily: 'monospace' }}>{format(d, 'EEEE d MMM yyyy', { locale: fr })}</p>
                            </div>
                          </div>
                        )
                      })}
                      ...
                      return (
                        <div key={c.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 14px', background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.05)', borderRadius: 10 }}>
                          <div style={{ flex: 1, minWidth: 0 }}>
                            ...
                            <p style={{ fontSize: 10, color: 'rgba(255,255,255,0.3)', margin: '2px 0 0', fontFamily: 'monospace' }}>Qté : {c.quantity}{c.details && ` · ${c.details}`}</p>
                          </div>
