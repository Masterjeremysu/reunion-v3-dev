  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: 'var(--color-bg-app)', overflow: 'hidden' }}>
      ...
      <div style={{ flexShrink: 0, padding: '0 24px', height: 52, borderBottom: '1px solid var(--color-border)', background: 'var(--color-bg-sidebar)', display: 'flex', alignItems: 'center', gap: 12 }}>
        <h1 style={{ fontSize: 14, fontWeight: 600, color: 'var(--color-text-main)', margin: 0 }}>Équipe</h1>
        ...
        <div style={{ marginLeft: 'auto', display: 'flex', gap: 8 }}>
          <button onClick={() => setShowInactive(!showInactive)}
            style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '6px 12px', background: showInactive ? 'var(--color-border)' : 'var(--color-border)', border: `1px solid ${showInactive ? 'var(--color-text-faded)' : 'var(--color-border)'}`, borderRadius: 8, color: showInactive ? 'var(--color-text-main)' : 'var(--color-text-muted)', fontSize: 12, cursor: 'pointer' }}>
      ...
      <div style={{ display: 'flex', flex: 1, minHeight: 0 }}>
        ...
        <div style={{ flex: 1, overflowY: 'auto', background: 'var(--color-bg-app)' }}>
          {!selected ? (
            ...
            <div>
              {/* Hero */}
              <div style={{ padding: '28px 32px 20px', borderBottom: '1px solid var(--color-border)', background: `linear-gradient(135deg, ${isInactive ? 'var(--color-border)' : accentColor + '08'} 0%, transparent 60%)`, position: 'relative' }}>
                {isInactive && (
