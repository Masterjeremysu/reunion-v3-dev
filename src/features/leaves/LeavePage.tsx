...
function useLeaveRequests() {
  ...
  return useQuery({
    queryKey: ['leave_requests', organization?.id],
...
function useLeaveBalances() {
  const { organization } = useAuth()
  return useQuery({
    queryKey: ['leave_balances', organization?.id],
    enabled: !!organization?.id,
    queryFn: async () => {
      const { data, error } = await supabase
        .from('leave_balances')
...
function CreateRequestForm({ colleagues, onClose }: { colleagues: any[]; onClose: () => void }) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 60, display: 'flex', alignItems: 'center', justifyContent: 'flex-end', background: 'var(--color-overlay)' }}
      ...
      <div style={{ height: '100%', width: 480, background: '#0d1018', borderLeft: '1px solid var(--color-border)', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
...
export function LeavePage() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: 'var(--color-bg-app)', overflow: 'hidden' }}>
      ...
      {/* Topbar */}
      <div style={{ flexShrink: 0, padding: '0 24px', height: 52, borderBottom: '1px solid var(--color-border)', background: 'var(--color-bg-sidebar)', display: 'flex', alignItems: 'center', gap: 14 }}>
        <h1 style={{ fontSize: 14, fontWeight: 600, color: 'var(--color-text-main)', margin: 0 }}>Congés & Absences</h1>
