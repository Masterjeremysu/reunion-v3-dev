...
// ─── LocalStorage key ────────────────────────────────────────────────────────
const LAYOUT_STORAGE_KEY = 'dashboard_grid_layout_v2'
...
const DEFAULT_LAYOUT = {
  ...
  xs: [
    { i: 'activity',    x: 0, y: 0,  w: 4, h: 6, minW: 2, minH: 3 },
    { i: 'cr',          x: 0, y: 6,  w: 4, h: 6, minW: 2, minH: 3 },
    { i: 'perf',        x: 0, y: 12, w: 4, h: 5, minW: 2, minH: 2 },
    { i: 'mood',        x: 0, y: 17, w: 4, h: 5, minW: 2, minH: 2 },
    { i: 'meetings',    x: 0, y: 22, w: 4, h: 5, minW: 2, minH: 2 },
    { i: 'alerts',      x: 0, y: 27, w: 4, h: 5, minW: 2, minH: 2 },
    { i: 'leaves',      x: 0, y: 32, w: 4, h: 5, minW: 2, minH: 2 },
    { i: 'urgentact',   x: 0, y: 37, w: 4, h: 4, minW: 2, minH: 2 },
    { i: 'vehicles',    x: 0, y: 41, w: 4, h: 4, minW: 2, minH: 2 },
  ],
  xxs: [
    { i: 'activity',    x: 0, y: 0,  w: 2, h: 6, minW: 1, minH: 3 },
    { i: 'cr',          x: 0, y: 6,  w: 2, h: 6, minW: 1, minH: 3 },
    { i: 'perf',        x: 0, y: 12, w: 2, h: 5, minW: 1, minH: 2 },
    { i: 'mood',        x: 0, y: 17, w: 2, h: 5, minW: 1, minH: 2 },
    { i: 'meetings',    x: 0, y: 22, w: 2, h: 5, minW: 1, minH: 2 },
    { i: 'alerts',      x: 0, y: 27, w: 2, h: 5, minW: 1, minH: 2 },
    { i: 'leaves',      x: 0, y: 32, w: 2, h: 5, minW: 1, minH: 2 },
    { i: 'urgentact',   x: 0, y: 37, w: 2, h: 4, minW: 1, minH: 2 },
    { i: 'vehicles',    x: 0, y: 41, w: 2, h: 4, minW: 1, minH: 2 },
  ],
}
...
export function DashboardPage() {
  ...
  const stats = useMemo(() => {
    ...
    const meetingsThisMonth = (meetings as any[])?.filter((m: any) => { const d = new Date(m.date); return d >= monthStart && d <= monthEnd }).length ?? 0
    const meetingsPrevMonth = (meetings as any[])?.filter((m: any) => { const d = new Date(m.date); return d >= prevStart && d <= prevEnd }).length ?? 0
    const allActions = (actions as any[]) ?? []
    const openActions = allActions.filter((a: any) => a.status !== 'completed' && a.status !== 'cancelled')
    const lateActions = openActions.filter((a: any) => a.due_date && isOverdue(a.due_date))
    ...
    const avgMood = (moods as any[])?.length ? Math.round(((moods as any[]).slice(0, 10).reduce((a: any, b: any) => a + b.mood_score, 0) / Math.min(moods.length, 10)) * 10) / 10 : null
    const todayStr = format(now, 'yyyy-MM-dd')
    const todayAbsent = (leaves ?? []).filter((l: any) => l.start_date <= todayStr && l.end_date >= todayStr).length
    return {
  ...
  const weeklyData = useMemo(() => Array.from({ length: 8 }, (_, i) => {
    ...
    return {
      ...
      réunions: (meetings as any[])?.filter((m: any) => { const d = new Date(m.date); return d >= wStart && d <= wEnd }).length ?? 0,
      actions: (actions as any[])?.filter((a: any) => { if (!a.due_date) return false; const d = new Date(a.due_date); return d >= wStart && d <= wEnd }).length ?? 0,
      retards: (actions as any[])?.filter((a: any) => { if (!a.due_date || a.status === 'completed') return false; return isOverdue(a.due_date) && new Date(a.due_date) >= wStart && new Date(a.due_date) <= wEnd }).length ?? 0,
    }
  }), [meetings, actions])
    ...
    <div style={{ background: 'var(--color-bg-app)', minHeight: '100vh', overflowY: 'auto' }}>
      ...
      <div style={{ padding: '20px 20px' }}>
        ...
        >
          {/* Widget: Activité */}
          <div key="activity">
            <Widget editMode={editMode} index={1}>
              ...
              <div style={{ flex: 1, minHeight: 0 }}>
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={weeklyData} margin={{ top: 4, right: 0, bottom: 0, left: -28 }}>
                    ...
                    <XAxis dataKey="week" tick={{ fill: 'var(--color-text-chart)', fontSize: 10, fontFamily: 'monospace' }} axisLine={false} tickLine={false} />
                    <YAxis tick={{ fill: 'var(--color-text-chart)', fontSize: 10 }} axisLine={false} tickLine={false} allowDecimals={false} />
          ...
          {/* Widget: Analyse CR */}
          <div key="cr">
            <Widget editMode={editMode} index={2}>
          ...
          <div key="perf">
            <Widget editMode={editMode} index={3}>
          ...
          <div key="mood">
            <Widget editMode={editMode} index={4}>
              ...
              <div style={{ flex: 1, minHeight: 0 }}>
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={moodTrend} margin={{ top: 4, right: 0, bottom: 0, left: -32 }}>
                    ...
                    <XAxis dataKey="date" tick={{ fill: 'var(--color-text-chart)', fontSize: 9, fontFamily: 'monospace' }} axisLine={false} tickLine={false} />
                    <YAxis domain={[1, 5]} ticks={[1, 3, 5]} tick={{ fill: 'var(--color-text-chart)', fontSize: 9 }} axisLine={false} tickLine={false} />
                    <Tooltip content={<CustomTooltip />} />
          ...
          <div key="meetings">
            <Widget editMode={editMode} index={5}>
          ...
          {/* Widget: Alertes */}
          <div key="alerts">
            <Widget editMode={editMode} borderColor={alerts.some(a => a.level === 'critical') ? '#ef4444' : undefined} index={6}>
              ...
              <div style={{ overflowY: 'auto', flex: 1 }}>
                {alerts.length === 0 ? (
                  ...
                  <AlertItem key={a.id || Math.random()} level={a.level} title={a.title} sub={a.sub} action={a.action} onClick={() => navigate(a.route)} />
                ))}
              </div>
            </Widget>
          </div>
          ...
          {/* Widget: Congés */}
          <div key="leaves">
            <Widget editMode={editMode} index={7}>
          ...
          <div key="urgentact">
            <Widget editMode={editMode} index={8}>
          ...
          {/* Widget: Véhicules */}
          <div key="vehicles">
            <Widget editMode={editMode} index={9}>
