...
// ─── Types ────────────────────────────────────────────────────────────────────
type CRCategory = 'success' | 'failure' | 'sensitive' | 'relational' | 'sse' | 'improvement'
...
const CR_SECTIONS: {
  ...
  { key: 'improvement',label: 'Amélioration',    color: '#D4537E', bg: '#D4537E08', border: '#D4537E25', icon: TrendingUp,     placeholder: 'Ex: Proposition d\'outil, idée...' },
]
...
function CRItemRow({
  ...
  return (
    <div style={{ display: 'flex', gap: 8, alignItems: 'flex-start', padding: '8px 0', borderBottom: '1px solid var(--color-border)' }}>
      {/* Dot */}
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <input
          ...
          style={{
            ...
            borderRadius: 8, padding: '7px 12px', fontSize: 13, color: 'var(--color-text-main)',
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginTop: 5, position: 'relative' }}>
          <button
            ...
            style={{
              ...
              background: attributed ? `${color}15` : 'var(--color-border)',
              border: `1px solid ${attributed ? color + '35' : 'var(--color-border)'}`,
              color: attributed ? color : 'var(--color-text-faded)',
          ...
          {showPicker && (
            ...
            >
              <button
                ...
                style={{ width: '100%', padding: '8px 12px', display: 'flex', alignItems: 'center', gap: 8, background: !item.attributed_to ? 'var(--color-border)' : 'transparent', border: 'none', cursor: 'pointer', color: 'var(--color-text-muted)', fontSize: 12, textAlign: 'left' }}>
                <div style={{ width: 18, height: 18, borderRadius: '50%', background: 'var(--color-bg-input)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              ...
              {colleagues.map(c => (
                <button
                  ...
                  style={{ width: '100%', padding: '8px 12px', display: 'flex', alignItems: 'center', gap: 8, background: item.attributed_to === c.id ? `${color}12` : 'transparent', border: 'none', cursor: 'pointer', color: item.attributed_to === c.id ? color : 'var(--color-text-main)', fontSize: 12, textAlign: 'left' }}>
      ...
      <button
        ...
        style={{ padding: 5, background: 'none', border: 'none', cursor: 'pointer', color: 'var(--color-text-faded)', borderRadius: 6, display: 'flex', transition: 'color 0.15s', flexShrink: 0 }}
    ...
    <div style={{ borderBottom: '1px solid var(--color-border)' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
        <button
          ...
          <span style={{ fontSize: 13, fontWeight: 600, color: open ? section.color : 'var(--color-text-main)', transition: 'color 0.15s' }}>{section.label}</span>
          {filled > 0 && (
          ...
          <ChevronDown style={{ width: 14, height: 14, color: 'var(--color-text-muted)', transform: open ? 'rotate(180deg)' : 'none', transition: 'transform 0.2s', marginLeft: 'auto' }} />
        </button>
        <button
...
export function NewMeetingModal({ onClose }: { onClose: () => void }) {
  ...
  const [crItems, setCrItems] = useState<Record<CRCategory, CRItem[]>>({
    success: makeItems(), failure: makeItems(), sensitive: makeItems(), relational: makeItems(), sse: makeItems(), improvement: makeItems(),
  })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const meeting = await createMeeting.mutateAsync({
        ...
        sse: crItems.sse.filter(i => i.content.trim()).map(i => i.content.trim()),
        improvements: crItems.improvement.filter(i => i.content.trim()).map(i => i.content.trim()),
      ...
      const categoryMap: Record<CRCategory, string> = {
        success: 'success', failure: 'failure', sensitive: 'sensitive', relational: 'relational', sse: 'sse', improvement: 'improvement'
      }
      Object.entries(crItems).forEach(([cat, items]) => {
        items.filter(i => i.content.trim()).forEach((item, idx) => {
          itemsToInsert.push({
            ...
            colleague_id: item.attributed_to || null,
            order_index: idx,
          })
        })
      })
  ...
  const fieldStyle = {
    width: '100%', background: 'var(--color-bg-input)', border: '1px solid var(--color-border2)',
    borderRadius: 10, padding: '10px 14px', fontSize: 13, color: 'var(--color-text-main)', outline: 'none',
        ...
        <div style={{ padding: '20px 24px 16px', borderBottom: '1px solid var(--color-border)', flexShrink: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
            <div>
              ...
              <h2 style={{ fontSize: 18, fontWeight: 700, color: 'var(--color-text-main)', margin: '3px 0 0', letterSpacing: '-0.02em' }}>Créer un compte-rendu</h2>
            </div>
            ...
            {STEPS.map((s, i) => {
              ...
              return (
                <button key={s.key} type="button"
                  ...
                  style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 5, padding: '6px 8px', borderRadius: 8, border: `1px solid ${isCurrent ? '#1D9E75' : isDone ? '#1D9E7530' : 'var(--color-border)'}`, background: isCurrent ? '#1D9E7512' : isDone ? '#1D9E7508' : 'transparent', cursor: 'pointer', transition: 'all 0.15s' }}>
                  {isDone ? <Check style={{ width: 11, height: 11, color: '#1D9E75' }} /> : <s.icon style={{ width: 11, height: 11, color: isCurrent ? '#1D9E75' : 'var(--color-text-faded)' }} />}
                  <span style={{ fontSize: 11, fontWeight: isCurrent ? 600 : 400, color: isCurrent ? '#5DCAA5' : isDone ? '#1D9E75' : 'var(--color-text-muted)', fontFamily: 'monospace' }}>{s.label}</span>
                </button>
              )
            })}
          </div>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '20px 24px' }}>
          ...
          {step === 'team' && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              <p style={{ fontSize: 12, color: 'var(--color-text-muted)', marginBottom: 8, fontFamily: 'monospace' }}>
              ...
              {activeColleagues.length === 0 && (
                <p style={{ fontSize: 13, color: 'var(--color-text-faded)', textAlign: 'center', padding: '24px 0' }}>Aucun collègue actif</p>
              )}
              {activeColleagues.map((c: any) => {
                ...
                return (
                  <button key={c.id} type="button" onClick={() => toggleColleague(c.id)}
                    style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 14px', background: selected ? '#1D9E7508' : 'var(--color-border)', border: `1px solid ${selected ? '#1D9E7540' : 'var(--color-border)'}`, borderRadius: 10, cursor: 'pointer', transition: 'all 0.15s' }}>
                    <ColleagueAvatar name={c.name} size={32} />
                    <div style={{ flex: 1, textAlign: 'left' }}>
                      <p style={{ fontSize: 13, fontWeight: 600, color: selected ? 'var(--color-text-main)' : 'var(--color-text-main)', margin: 0 }}>{c.name}</p>
                      <p style={{ fontSize: 11, color: 'var(--color-text-muted)', margin: '1px 0 0', fontFamily: 'monospace' }}>{c.post}</p>
                    </div>
                    <div style={{ width: 18, height: 18, borderRadius: '50%', border: `2px solid ${selected ? '#1D9E75' : 'var(--color-text-faded)'}`, background: selected ? '#1D9E75' : 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all 0.15s', flexShrink: 0 }}>
                      {selected && <Check style={{ width: 10, height: 10, color: 'var(--color-text-main)' }} />}
                    </div>
                  </button>
                )
              })}
        ...
        <div style={{ padding: '16px 24px', borderTop: '1px solid var(--color-border)', display: 'flex', gap: 10, flexShrink: 0 }}>
          {step !== 'info' && (
            <button type="button"
              ...
              style={{ padding: '11px 16px', fontSize: 13, color: 'var(--color-text-muted)', background: 'var(--color-bg-input)', border: '1px solid var(--color-border2)', borderRadius: 10, cursor: 'pointer' }}>
              ← Retour
            </button>
          )}
          ...
          {step !== 'cr' ? (
            <button type="button"
              ...
              style={{ flex: 1, padding: '11px 0', fontSize: 13, fontWeight: 600, color: 'var(--color-text-main)', background: '#1D9E75', border: 'none', borderRadius: 10, cursor: 'pointer', opacity: step === 'info' && !title.trim() ? 0.4 : 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
              Suivant →
            </button>
          ) : (
            <button
              type="button" onClick={handleSubmit as any} disabled={submitting || !title.trim()}
              style={{ flex: 1, padding: '11px 0', fontSize: 13, fontWeight: 700, color: 'var(--color-text-main)', background: '#1D9E75', border: 'none', borderRadius: 10, cursor: 'pointer', opacity: !title.trim() ? 0.4 : 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
