...
import {
  ...
  ShoppingCart, Car, Activity, Calendar, LogOut, AlertTriangle,
  Clock, Zap
} from 'lucide-react'
...
function NavBadge({ count, level }: { count: number; level: BadgeLevel }) {
  ...
  return (
    <span style={{
      ...
      background: s.bg, color: s.color, borderRadius: 20, padding: '0 5px',
...
function PulsingDot({ level }: { level: 'critical' | 'warn' }) {
  ...
  return (
    <span style={{
      position: 'absolute', top: 8, right: 14, width: 6, height: 6,
    ...
    <div style={{ display: 'flex', height: '100vh', background: '#0a0c12', overflow: 'hidden' }}>
      ...
      <aside style={{
        ...
        <div style={{ padding: '20px 20px 16px', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
          ...
          {hasCritical && (
            <div style={{
              ...
              background: '#E24B4A10', border: '1px solid #E24B4A25',
        ...
        <div style={{ padding: '12px 16px', borderTop: '1px solid rgba(255,255,255,0.05)', display: 'flex', alignItems: 'center', gap: 10 }}>
          <div style={{
            width: 28, height: 28, borderRadius: '50%', background: '#1D9E7530',
            display: 'flex', alignItems: 'center', justifyContent: 'center',
            fontSize: 11, fontWeight: 600, color: '#5DCAA5', flexShrink: 0,
          }}>
            {initials}
          </div>
          <div style={{ flex: 1, minWidth: 0 }}>
            <p style={{ fontSize: 11, fontWeight: 500, color: '#e8eaf0', margin: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
              {user?.email}
            </p>
          </div>
...
function NavSection({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div style={{ marginBottom: 4 }}>
      <p style={{
        padding: '10px 20px 4px', fontSize: 9, fontWeight: 600,
        letterSpacing: '0.12em', textTransform: 'uppercase',
        color: 'rgba(255,255,255,0.2)', fontFamily: 'monospace', margin: 0,
      }}>
        {label}
      </p>
      {children}
